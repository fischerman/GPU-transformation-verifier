...
def copy : mclk sig := mclk.shared_assign "b" v[@tlocal_var sig "j" (λ_, 0) rfl] rfl rfl (@shared_var sig "a" (v[@tlocal_var sig "j" (λ_, 0) rfl]).nth rfl)

def e₁ := 10
def f₁ := 10
def p₁ : mclp sig := mclp.intro (λ m, e₁) (
    mclk.for "i" rfl rfl (literal_int 0) (@tlocal_var sig "i" (λ_, 0) rfl < literal_int f₁) (mclk.tlocal_assign "i" v[0] rfl rfl $ tlocal_var "i" (λ_, 0) rfl + literal_int 1) (
        mclk.tlocal_assign "j" v[literal_int 0] rfl rfl (@tlocal_var sig "tid" (λ_, 0) rfl * literal_int e₁ + @tlocal_var sig "i" (λ_, 0) rfl) ;;
        copy
    )
)
...
def f₂ := 20
def p₂ : mclp sig := mclp.intro (λ m, e₂) (
    mclk.for "i" rfl rfl (literal_int 0) (@tlocal_var sig "i" (λ_, 0) rfl < literal_int f₂) (mclk.tlocal_assign "i" v[0] rfl rfl $ @tlocal_var sig "i" (λ_, 0) rfl + literal_int 1) (
        mclk.tlocal_assign "j" v[literal_int 0] rfl rfl (@tlocal_var sig "tid" (λ_, 0) rfl * literal_int e₂ + @tlocal_var sig "i" (λ_, 0) rfl) ;;
        copy
    )
)

def read_tid {sig : signature} := @tlocal_var sig "tid" (λ_, 0) sig.property.right.right
...
rhl.mclp_rel eq 
(mclp.intro (λ m, e₁) (
    @mclk.tlocal_assign sig type.int _ "k" v[literal_int 0] h₁ h₂ read_tid ;;
    k₁
))
(mclp.intro (λ m, e₂) (
    mclk.for "k" h₁ h₂ read_tid ((by rw ← h₁; exact @tlocal_var sig "k" (λ_, 0) h₃) < literal_int e₂) (mclk.tlocal_assign "k" v[0] h₁ h₂ $ tlocal_var "k" (λ_, 0) h₃ + literal_int e₂)
        k₂
)) Q := begin
